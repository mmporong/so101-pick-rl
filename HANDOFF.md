...
전체 성공은 0/600이다. 525회는 코드상 들기 판정 후 목표 배치에 실패했다.
이 들기 판정은 정상적인 기계적 파지를 보장하지 않는다. 아래 2026-09-13 접촉 진단을 우선 읽는다.
...
## 2026-09-13 파지 접촉 진단 (최신)

사용자가 영상의 닫힌 그리퍼 끼움과 상향 손목 자세를 지적해, 정책·보상·물체 크기를
바꾸지 않고 최신 model_389의 접촉점·법선력·분리거리를 계측했다.
`--grasp_audit`는 고정 정책 추론 전용이며 학습/optimizer 업데이트가 없다.
실제 그리퍼·jaw 충돌 프림에는 MeshMerge와 convexDecomposition 설정이 있다.
계측을 켜고 끈 동일 seed의 첫 90전이에서 관절·큐브·정책 명령·접촉력·단계 상태가 정확히 일치했다.

첫 seed에서 힘을 받는 gripper–cube 접촉의 PhysX 분리거리가 약 -25.4mm로 기록됐다.
따라서 과거 영상의 '유효한 들기'와 525/600은 **기존 코드 기준을 충족한 수치**로만 읽는다.
정상적인 열기→접근→닫기→운반→열기 또는 실물에서 가능한 파지의 증거로 사용하지 않는다.
상세 결과와 원본 경로는 [접촉 진단 보고서](reports/windows/pp_grasp_contact_audit_20260913.json)에 있다.

표본은 정책 경계 30Hz이며 매번 마지막 120Hz 물리 substep의 접촉 스트림만 읽는다.
손가락 authored mesh 꼭짓점 높이는 cooked convex hull의 정확한 침투 깊이와 다르다.
충돌 형상·구동 토크·solver 설정 중 무엇이 겹침의 원인인지는 아직 분리 검증하지 않았다.
이번 변경은 진단 도구이며 정책 개선 완료가 아니다. 이전 배치/해제 개선 순서를 곧바로
실행하기 전에 파지의 물리적 타당성을 먼저 검증해야 한다.

## 2026-09-13 시네마틱 재촬영 완료 (이전 기록)
